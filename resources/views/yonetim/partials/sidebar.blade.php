<aside class="fixed inset-y-0 left-0 lg:static bg-white border-r border-slate-200 flex flex-col z-50 transition-all duration-300 h-full -translate-x-full lg:translate-x-0" 
    ...
    <nav class="flex-1 overflow-y-auto p-4 space-y-1 custom-scrollbar">
        ...
        <!-- Başvurular -->
        <div x-data="{ open: {{ (request()->routeIs('yonetim.mesajlar.*') || request()->routeIs('yonetim.teklifler.*') || request()->routeIs('yonetim.bayilik.*')) ? 'true' : 'false' }} }">
            ...
            <div x-show="open && sidebarOpen" x-collapse x-cloak class="mt-1 space-y-1 ml-4 border-l border-slate-100 pl-2">
                ...
                <a href="{{ route('yonetim.bayilik.index') }}" 
                   class="flex items-center gap-3 px-3 py-2 rounded-lg transition-all whitespace-nowrap {{ request()->routeIs('yonetim.bayilik.*') ? 'bg-blue-50 text-blue-600 font-bold' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900 text-sm' }}">
                     <i data-lucide="store" class="w-4 h-4 flex-shrink-0"></i>
                     <span>Bayilik Başvuruları</span>
                     @if(($unreadDealerCount ?? 0) > 0)
                         <span class="ml-auto bg-blue-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full">{{ $unreadDealerCount }}</span>
                     @endif
                </a>
