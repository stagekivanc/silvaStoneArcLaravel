...
@section('title', ($product['title'] ?? 'Ürün') . ' | Silva Arc Panel')
@section('meta_description', $product['lead'] ?: (($product['title'] ?? '') . ' — Silva Arc Panel dekoratif duvar paneli.'))
...
@section('content')
  <section class="pdp">
    <div class="mx-auto max-w-[1440px] px-5 md:px-8">
      <nav class="pdp-crumb">
        <a href="{{ $listUrl }}">{{ data_get($detail, 'collection_crumb', 'Silva Arc Panel Koleksiyonu') }}</a>
        <span>/</span>
  ...
  <section class="pdp-related">
    <div class="mx-auto max-w-[1440px] px-5 md:px-8">
      <p class="page-intro-kicker font-display italic">{{ data_get($detail, 'related_kicker', 'Silva Arc Panel Koleksiyonu') }}</p>
      <h2>{{ $relatedFrom }}</h2>
  ...
  <script>
    (function () {
      ...
      document.getElementById('pdp-add')?.addEventListener('click', function () {
        if (!window.SilvaCart || typeof window.SilvaCart.add !== 'function') return;
