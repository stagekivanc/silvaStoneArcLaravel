...
if (!function_exists('seo_json_ld')) {
    ...
    function seo_json_ld(array $context = []): array
    {
        ...
        if ($product) {
            $title = trim((string) ($product->seo_title ?: (trim(($product->name ?? '') . ' | Silva Arc Panel') ?: $title)));
        ...
        if (count($segments) > 0) {
            ...
            if ($pageTranslation) {
                ...
                $defaults = [
                    ...
                    'dealer'        => ['tr' => 'bayilik-basvuru', 'en' => 'dealer-application', 'ru' => 'diler'],
...
if (!function_exists('menu_page_url')) {
    function menu_page_url(string $type): string
    {
        $lang = app()->getLocale();
        $knownTypes = [
            'corporate', 'contact', 'dealer', 'application', 'solution', 'sustainability', 'kvkk', 'cookie-policy', 'terms',
            'products',
        ];
