...
class HomeController extends Controller
{
    ...
    {
        $page = \App\Models\Page::where('type', 'dealer')->firstOrFail();
        $dealer = page_extras($page, \App\Support\SilvaDealerDefaults::data());

        return view('frontend.dealer', compact('page', 'dealer'));
    }

    public function storeDealer(\Illuminate\Http\Request $request)
    {
        $throttleKey = 'dealer_submit|' . $request->ip();
        if (RateLimiter::tooManyAttempts($throttleKey, 3)) {
            return back()->with('error', form_t('form_dealer_rate_limit', 'Çok fazla başvuru gönderdiniz. Lütfen biraz bekleyin.'));
        }
        RateLimiter::hit($throttleKey, 60);

        if ($request->filled('hp_fax')) {
            return back()->with('success', form_t('form_dealer_success_short', 'Başvurunuz başarıyla iletildi.'));
        }

        $dealerDefaults = \App\Support\SilvaDealerDefaults::data();
        ...
        $validated = $request->validate([
            ...
            'tax_department' => 'required|string|max:255',
            'tax_no' => 'required|string|max:64',
            'field_of_activity' => 'nullable|string|max:255',
            'references' => 'nullable|string|max:500',
            'dealer_type' => 'required|in:domestic,abroad',
            'message' => 'nullable|string|max:5000',
            'consent' => 'accepted',
            'g-recaptcha-response' => [recaptcha_enabled() ? 'required' : 'nullable', new \App\Rules\ReCaptcha],
        ], [
            'name.required' => form_t('form_dealer_name_required', 'Ad alanı zorunludur.'),
            'surname.required' => form_t('form_dealer_surname_required', 'Soyad alanı zorunludur.'),
            'company_name.required' => form_t('form_dealer_company_required', 'Firma adı alanı zorunludur.'),
            'company_address.required' => form_t('form_dealer_address_required', 'Firma adresi zorunludur.'),
            'city.required' => form_t('form_dealer_city_required', 'Şehir alanı zorunludur.'),
            'town.required' => form_t('form_dealer_district_required', 'İlçe alanı zorunludur.'),
            'phone_number.required' => form_t('form_dealer_phone_required', 'Telefon numarası zorunludur.'),
            'email_address.required' => form_t('form_dealer_email_required', 'E-posta alanı zorunludur.'),
            'email_address.email' => form_t('form_dealer_email_invalid', 'Geçerli bir e-posta adresi giriniz.'),
            'tax_department.required' => form_t('form_dealer_tax_office_required', 'Vergi dairesi zorunludur.'),
            'tax_no.required' => form_t('form_dealer_tax_no_required', 'Vergi no zorunludur.'),
            'dealer_type.required' => form_t('form_dealer_type_required', 'Bayilik türü seçiniz.'),
            'consent.accepted' => form_t('form_dealer_consent_required', 'Devam etmek için aydınlatma metnini kabul etmelisiniz.'),
            'g-recaptcha-response.required' => form_t('form_recaptcha_required', 'Lütfen robot olmadığınızı doğrulayın.'),
        ]);

        $fullName = trim($validated['name'] . ' ' . $validated['surname']);

        \App\Models\DealerApplication::create([
            'name' => $validated['name'],
            'surname' => $validated['surname'],
            'company' => $validated['company_name'],
            'address' => $validated['company_address'],
            'city' => $validated['city'],
            'district' => $validated['town'],
            'phone' => $validated['phone_number'],
            'email' => $validated['email_address'],
            'website' => $validated['website'] ?? null,
            'tax_department' => $validated['tax_department'],
            'tax_no' => $validated['tax_no'],
            'field_of_activity' => $validated['field_of_activity'] ?? null,
            'company_references' => $validated['references'] ?? null,
        ...
        try {
            $siteName = \App\Models\Setting::get('site_name', 'Silva Arc Panel');
            $typeLabel = $validated['dealer_type'] === 'abroad' ? 'Yurtdışı bayilik' : 'Yurtiçi bayilik';

            $details = [
                'İsim Soyisim' => $fullName,
                'Firma adı' => $validated['company_name'],
                'Adres' => $validated['company_address'],
                'Şehir' => $validated['city'],
                'İlçe' => $validated['town'],
                'Telefon' => $validated['phone_number'],
                'E-posta' => $validated['email_address'],
                'Websitesi' => $validated['website'] ?: '—',
                'Vergi dairesi' => $validated['tax_department'],
                'Vergi no' => $validated['tax_no'],
                'Faaliyet alanı' => $validated['field_of_activity'] ?: '—',
                'Referanslar' => $validated['references'] ?: '—',
                'Bayilik türü' => $typeLabel,
                'Not' => $validated['message'] ?: '—',
            ];

            \App\Services\MailjetService::sendToAdmins(
                'Yeni Bayilik Başvurusu: ' . $validated['company_name'],
                view('emails.generic', [
                    'subject' => 'Yeni Bayilik Başvurusu',
                    'title' => 'Bayilik formundan yeni bir başvuru geldi.',
                    'message_text' => 'Başvuru detayları aşağıdadır:',
                    'details' => $details,
                ])->render()
            );
            ...
            \App\Services\MailjetService::send(
                ...
                $validated['name'],
                'Bayilik Başvurunuz Alındı - ' . $siteName,
                view('emails.generic', [
                    'subject' => 'Bayilik Başvurunuz Alındı',
                    'title' => 'Merhaba ' . $validated['name'] . ',',
                    'message_text' => 'Bayilik başvurunuz başarıyla tarafımıza ulaşmıştır. En kısa sürede size dönüş sağlayacağız.',
                    'details' => [
                        'Firma' => $validated['company_name'],
                        'Bayilik türü' => $typeLabel,
                        'Tarih' => date('d.m.Y H:i'),
                    ],
                ])->render()
            ...
            \Illuminate\Support\Facades\Log::error('Mail sending error in storeDealer: ' . $e->getMessage());
        }

        $success = data_get($dealerDefaults, 'form.success_message')
            ?: form_t('form_dealer_success', 'Teşekkürler — başvurunuz alındı. En kısa sürede sizinle iletişime geçeceğiz.');
    ...
    public function dynamicPage($lang, $slug)
    {
        ...
        if ($page->type == 'dealer') {
            $extraData['dealer'] = page_extras($page, \App\Support\SilvaDealerDefaults::data());
        }
    ...
    public function moduleDispatcher($lang, $module, $slug = null)
    {
        ...
        if ($matchedTranslation) {
            ...
            $defaults = [
                ...
                'iletisim' => 'contact', 'bayilik-basvuru' => 'dealer', 'teklif-al' => 'get_quote', 'b2b-toptan' => 'get_quote', 'kurumsal' => 'corporate', 'hakkimizda' => 'corporate',
                ...
                'contact' => 'contact', 'dealer-application' => 'dealer', 'get-quote' => 'get_quote', 'b2b-wholesale' => 'get_quote', 'corporate' => 'corporate', 'about' => 'corporate', 'about-us' => 'corporate',
        ...
        if ($moduleKey && isset($methodMap[$moduleKey])) {
            ...
            try {
                ...
                if (!$slug) {
                    if ($method === 'productDetail') {
                        return $this->products(null);
                    }

                    $noSlugMethods = ['contact', 'dealer', 'corporate', 'application', 'privacyPolicy', 'returnPolicy', 'shippingPolicy', 'references', 'faq', 'login'];
