...
class SettingController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        if ($active_tab == 'recaptcha') {
            $siteKey = trim((string) $request->input('recaptcha_site_key', ''));
            $secretKey = trim((string) $request->input('recaptcha_secret_key', ''));

            // Anahtarlar girildiyse otomatik aktif, boşsa kapalı
            if ($siteKey !== '' && $secretKey !== '') {
                \App\Models\Setting::set('recaptcha_status', 1);
            } else {
