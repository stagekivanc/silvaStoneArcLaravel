    ...
    public function boot(): void
    {
        ...
        if (!app()->runningInConsole()) {
            ...
            \Illuminate\Support\Facades\View::composer('yonetim.*', function ($view) {
                ...
                $dList = \App\Models\DealerApplication::where('is_read', false)->latest()->limit(10)->get();
                ...
                $unreadDealers = $dList->map(function($item) {
                    $item->notif_type = 'dealer';
                    $item->notif_icon = 'store';
                    $item->notif_label = 'Yeni Bayilik: ' . $item->full_name;
                    $item->notif_route = route('yonetim.bayilik.show', $item->id);
                    return $item;
                });

                $notifications = $unreadMessages->concat($unreadOffers)->concat($unreadDealers)
                ...
                $countDealers = \App\Models\DealerApplication::where('is_read', false)->count();

                $view->with([
                    'unreadMessagesCount' => $countMessages,
                    'unreadOffersCount' => $countOffers,
                    'unreadDealerCount' => $countDealers,
                    'totalUnreadCount' => $countMessages + $countOffers + $countDealers,
