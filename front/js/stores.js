(function SilvaStores() {
  ...
  if (rails.length) {
    ...
    const setHtml = (list) =>
      list
        .map(
          ([id, label]) =>
            `<a class="store-city-name" href="magazalar.html">${esc(label)}</a>`
        )
        .join('');
    const fill = (el, list) => {
  ...
  if (grid.closest('.store-marquee')) {
    const clone = document.createElement('div');
    clone.innerHTML = cardsHtml;
    clone.querySelectorAll('.store-card').forEach((card) => {
      card.setAttribute('aria-hidden', 'true');
      card.querySelectorAll('a').forEach((link) => link.setAttribute('tabindex', '-1'));
    });
    grid.insertAdjacentHTML('beforeend', clone.innerHTML);
  }
})();
