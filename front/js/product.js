(function productDetail() {
  ...
  const spec = window.silvaSpec(product);
  const sizeLabel = String(product.size || '').replace(/x/gi, '×');
  const thick = String(product.thick || '').replace('-', '–');
  const pageUrl = `https://silvastone.acarkon.com/${href(product)}`;

  document.title = `${name} | Silva Arc Panel`;
  document.querySelector('meta[name="description"]')?.setAttribute(
    'content',
    `${name} — ${catName} koleksiyonu. Silva Arc Panel.`
  );
  document.querySelector('link[rel="canonical"]')?.setAttribute('href', pageUrl);
  document.querySelector('meta[property="og:title"]')?.setAttribute('content', `${name} | Silva Arc Panel`);
  document.querySelector('meta[property="og:url"]')?.setAttribute('href', pageUrl);

  document.getElementById('pdp-crumb').innerHTML = `
    <a href="urunler.html">Silva Arc Panel Koleksiyonu</a>
    <span>/</span>
  ...
  document.getElementById('pdp-info').innerHTML = `
    ...
    <div class="pdp-chips">
      ...
      ${sizeLabel ? `<span>${esc(sizeLabel)} mm</span>` : ''}
      ${thick ? `<span>${esc(thick)} mm</span>` : ''}
      ${product.material ? `<span>${esc(product.material)}</span>` : ''}
      <span>İç mekân</span>
    ...
    <dl class="pdp-specs">
      ...
      <div><dt>Ölçü</dt><dd>${esc(sizeLabel || '—')} mm</dd></div>
      <div><dt>İncelik</dt><dd>${esc(thick || '—')} mm</dd></div>
      <div><dt>Kullanım</dt><dd>İç mekân</dd></div>
      ${product.material ? `<div><dt>Malzeme</dt><dd>${esc(product.material)}</dd></div>` : ''}
      <div><dt>Özel sipariş</dt><dd>${esc(product.sizeExtra || 'Talep üzerine')}</dd></div>
    </dl>
    <div class="pdp-buy">
  ...
  const syncQuote = () => {
    ...
    const message = [
      'Merhaba,',
      '',
      'Silva Arc Panel web sitesi üzerinden ulaşıyorum.',
      '',
    ...
    .map(
      (p) => `<article class="plp-card">
        ...
        <div class="plp-card-info">
          ...
          <p class="plp-card-spec">${esc(colors[p.color] || '')}${window.silvaSpec(p) ? ` · ${esc(window.silvaSpec(p))}` : ''}</p>
        </div>
      </article>`
    )
    .join('');
