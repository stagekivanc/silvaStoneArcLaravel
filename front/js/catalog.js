(function catalogListing() {
  ...
  const defaults = { cat: 'all', color: 'all', size: '', thick: '', depot: '', q: '', page: 1, cols: 3, per: 24 };
  ...
  const readState = () => {
    ...
    return {
      ...
      size: q.get('size') === '60x280' ? q.get('size') : '',
      thick: q.get('thick') === '8' ? '8' : '',
  ...
  const media = (p) => {
    ...
    if (!gallery.length) return `<span class="plp-ph">Ürün resmi hazırlanıyor</span>`;
    const ticks =
      gallery.length > 1
        ? `<span class="plp-hover-bar" aria-hidden="true">${gallery
            ...
            .join('')}</span>`
        : '';
    return `<img src="${esc(gallery[0])}" alt="${esc(p.name)}" loading="lazy" onerror="this.outerHTML='<span class=&quot;plp-ph&quot;>Ürün resmi hazırlanıyor</span>'" />${ticks}`;
  };
  ...
  const sync = (s) => {
    ...
    root.classList.toggle(
      'is-filtered',
      s.cat !== 'all' || s.color !== 'all' || Boolean(s.size || s.thick || s.depot || s.q)
    );
    const extra = [s.color !== 'all', Boolean(s.size), Boolean(s.thick), Boolean(s.depot)].filter(Boolean).length;
  ...
  const render = () => {
    ...
    countEl.textContent = `${list.length} yüzey`;
    const emptyCopy = document.querySelector('#product-empty p');
    const emptyBtn = document.getElementById('product-empty-reset');
    if (emptyCopy) {
      emptyCopy.textContent = s.q ? `“${s.q}” ile eşleşen ürün yok.` : 'Bu seçime uygun ürün yok.';
    }
    if (emptyBtn) emptyBtn.textContent = s.q ? 'Aramayı temizle' : 'Filtrelemeyi sıfırla';
